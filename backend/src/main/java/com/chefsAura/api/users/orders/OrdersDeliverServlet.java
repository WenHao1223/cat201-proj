...
// @WebServlet("/api/users/orders/deliver")
public class OrdersDeliverServlet extends HttpServlet {
    @Override
    public void init() throws ServletException {
        super.init();
        System.out.println("OrdersDeliverServlet initialized.");
    }
    ...
    public void doPut(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("OrdersDeliverServlet PUT request received with parameters: email = " + email
                + ", orderID = " + orderID);
                ...
                try {
                    ...
                    for (Order order : orders) {
                        if (order.getOrderID() == orderID) {
                            ...
                            jsonResponse.add("deliveredOrder", cancelledOrderJson);

                            user.deliverOrder(orderID);
                            ...
                            for (Order orderItem : orders) {
                                ...
                                try {
                                    JsonObject paymentJson = new JsonObject();
                                    Payment payment = user.getPaymentDetailsByID(orderItem.getPaymentID());
                                    paymentJson.addProperty("paymentID", payment.getPaymentID());
                                    paymentJson.addProperty("paymentMethod", payment.getPaymentMethod().getMethod());
                                    paymentJson.addProperty("cardNumber", payment.getLastFourDigits());
